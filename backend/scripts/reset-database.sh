...
if [ -d "backend/uploads" ]; then
    find backend/uploads -type f -name "*.mp4" -delete 2>/dev/null || true
    find backend/uploads -type f -name "*.mov" -delete 2>/dev/null || true
    find backend/uploads -type f -name "*.avi" -delete 2>/dev/null || true
    log_success "Arquivos de upload limpos"
fi

# Remover arquivos de cortes (mantendo a pasta)
if [ -d "backend/cuts" ]; then
    find backend/cuts -type f -name "*.mp4" -delete 2>/dev/null || true
    find backend/cuts -type f -name "*.mov" -delete 2>/dev/null || true
    find backend/cuts -type f -name "*.avi" -delete 2>/dev/null || true
...
if ! npx prisma migrate reset --schema=backend/prisma/schema.prisma --force --skip-generate; then
    log_warning "Reset com migrate falhou, tentando db push..."
    
    # Alternativa: apagar e recriar manualmente
    if ! npx prisma db push --schema=backend/prisma/schema.prisma --force-reset --accept-data-loss; then
...
if ! npx prisma db push --schema=backend/prisma/schema.prisma --accept-data-loss; then
...
mkdir -p backend/uploads backend/cuts

# Recriar gitkeep files
touch backend/uploads/.gitkeep
touch backend/cuts/.gitkeep
