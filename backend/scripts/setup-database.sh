...
if [ ! -f "backend/prisma/schema.prisma" ]; then
    log_error "Schema do Prisma não encontrado em backend/prisma/schema.prisma"
    exit 1
fi
...
if npx prisma db pull --schema=backend/prisma/schema.prisma --print >/dev/null 2>&1; then
...
if [ ! -d "backend/uploads" ]; then
    log_error "Não foi possível criar pasta backend/uploads"
    exit 1
fi

if [ ! -d "backend/cuts" ]; then
    log_error "Não foi possível criar pasta backend/cuts"
    exit 1
fi
...
echo "   1. Configure suas APIs sociais no arquivo .env (veja backend/docs/API_SETUP.md)"
